...
let mobileConsole = (function() {
  ...
  function createElem(type, className, css) {
    ...
    let element = setCSS(document.createElement(type), css);
    if (className) {
  ...
  let setLineStyle = (function() {
      let lineStyles = function(style) {
        switch (style) {
          case 'log':
            return {
              text: {
    ...
    getLink = function(href, textString) {
      let HTMLurl = elements.link.cloneNode(false);
      if (href) {
    ...
    toggleHeight = function() {
      if (status.initialized) {
        ...
        let newHeight = elements.base.minimized ? getMaxHeight() + 'px' : '0px';
  ...
  function initConsoleElement() {
    ...
    function attach(console) {
      ...
      let existingPadding = isNaN(parseInt(document.body.style.paddingBottom, 10))
        ? 0
        : parseInt(document.body.style.paddingBottom, 10);
      setCSS(document.body, {
        paddingBottom:
    ...
    function toggleLogType(e) {
      let button = e.currentTarget || e.srcElement;
      let logType = button.innerHTML.toLowerCase();
      let elems = elements.lines[logType],
        i = elems.length;
  ...
  function initConsole() {
    ...
    function formatStackTrace(trace, origtrace) {
      ...
      let traceToProcess = origtrace && origtrace !== '' ? origtrace : stackTraceOrig,
        i,
        lines,
        url,
        txt,
    ...
    function colorizeData(key, value) {
      ...
      if (typeof value === 'string') {
        ...
        if (isEmpty(value)) {
          ...
          let tempTable = createElem('table', 'stackTraceSubTable', {
              border: '0 none',
              margin: 0,
              display: 'none',
              marginLeft: '10px',
    ...
    function toggleDetails(e) {
      ...
      if (button.getAttribute('toggles') === 'table') {
        let tables = button.parentElement.getElementsByTagName('table');
        i = tables.length;
        while (i--) {
          hidden =
    ...
    function interceptConsole(method) {
      let original = console ? console[method] : missingMethod(),
        i,
        stackTraceOrig;
      if (!console) {
        console = {};
  ...
  function initCommandLine() {
    ...
    function hoverAutoComplete(e) {
      ...
      let hovered = getFromArrayById('hover').element,
    ...
    function autoComplete(command) {
      ...
      let searchString = encodeURI(command),
        matches,
        match,
        row,
        i,
        ...
        if (tgt === elements.consoleinput) {
          if (e.key === 'Enter' || e.keyCode === 13) {
            ...
            if (status.acActive) {
              ...
              let hovered = getFromArrayById('hover').position;
              let counter = hovered === -1 ? elements.acItems.length : hovered;
              ...
              let hist = history.input.commands;
